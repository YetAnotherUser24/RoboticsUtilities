...
def plot_cylinder(ax, p0, p1, radius, resolution=6, alpha=1, color="g"):
    ...
    X1, Y1, Z1 = [
        p0[i] + v[i] * t + radius * np.sin(th2) * n1[i] + radius * np.cos(th2) * n2[i]
        for i in [0, 1, 2]
    ]
    # Coordinates for the bottom of the cylinder
    X2, Y2, Z2 = [
        p0[i] + rsample[i] * np.sin(th) * n1[i] + rsample[i] * np.cos(th) * n2[i]
        for i in [0, 1, 2]
    ]
    ...
    X3, Y3, Z3 = [
        p0[i]
        + v[i] * length
        + rsample[i] * np.sin(th) * n1[i]
        + rsample[i] * np.cos(th) * n2[i]
        for i in [0, 1, 2]
    ]
    # Plot
...
class SerialRobot(object):
    def __init__(self, L, name="robot"):
        ...
        self.p = (self.isdzero.count(False) + self.ndof) * [
            0,
        ]

    def _Tdh(self, k, d, th):
        cth = np.cos(self.th[k] + th)
        sth = np.sin(self.th[k] + th)
        ca = np.cos(self.alpha[k])
        sa = np.sin(self.alpha[k])
        return np.array(
            [
                [cth, -ca * sth, sa * sth, self.a[k] * cth],
                [sth, ca * cth, -sa * cth, self.a[k] * sth],
                [0.0, sa, ca, self.d[k] + d],
                [0.0, 0.0, 0.0, 1.0],
            ]
        )

    def _sym_Tdh(self, k):
        cth = sp.cos(self.th[k])
        sth = sp.sin(self.th[k])
        ca = sp.cos(self.alpha[k])
        sa = sp.sin(self.alpha[k])
        return sp.Matrix(
            [
                [cth, -ca * sth, sa * sth, self.a[k] * cth],
                [sth, ca * cth, -sa * cth, self.a[k] * sth],
    ...
    def fkine(self, q, verbose=False):
        ...
        for k in range(self.ndof):
            if self.type[k] == "r":
                T = self._Tdh(k, 0.0, q[k])
                if verbose:
                    print("\nT" + str(k) + str(k + 1) + ":")
                    print(np.round(T, 3))
            elif self.type[k] == "p":
                T = self._Tdh(k, q[k], 0.0)
                if verbose:
                    print("\nT" + str(k) + str(k + 1) + ":")
                    print(np.round(T, 3))
            else:
    ...
    def plot(
        ...
        if not self.isdzero[0]:
            if self.type[0] == "r":
                self.p[pcnt] = np.array([0.0, 0.0, self.d[0], 1.0])
                pcnt += 1
            elif self.type[0] == "p":
                self.p[pcnt] = np.array([0.0, 0.0, self.d[0] + q[0], 1.0])
                pcnt += 1
        ...
        for k in range(self.ndof):
            if self.type[k] == "r":
                ...
                if not self.isdzero[k] and k != 0:
                    self.p[pcnt] = np.array([0.0, 0.0, self.d[k], 1.0])
            elif self.type[k] == "p":
                T = self._Tdh(k, q[k], 0.0)
                if not self.isdzero[k] and k != 0:
                    self.p[pcnt] = np.array([0.0, 0.0, self.d[k] + q[k], 1.0])
            else:
        ...
        if ee:
            ...
            p2 = np.array([0, 0.1, 0.2, 1])
            p3 = np.array([0, -0.1, 0, 1])
            p4 = np.array([0, -0.1, 0.2, 1])
            p1 = self.Ts[-1].dot(p1)
            p2 = self.Ts[-1].dot(p2)
            p3 = self.Ts[-1].dot(p3)
            p4 = self.Ts[-1].dot(p4)
            # Plot an end effector
            ax.plot(
                [p1[0], p2[0]], [p1[1], p2[1]], [p1[2], p2[2]], color="k", linewidth=3
            )
            ax.plot(
                [p3[0], p4[0]], [p3[1], p4[1]], [p3[2], p4[2]], color="k", linewidth=3
            )
            ax.plot(
                [p1[0], p3[0]], [p1[1], p3[1]], [p1[2], p3[2]], color="k", linewidth=3
            )
        # Reference frame for the end effector (with respect to frame 0)
        ax.plot(
            [self.Ts[-1][0, 3], self.Ts[-1][0, 3] + ascale * self.Ts[-1][0, 0]],
            [self.Ts[-1][1, 3], self.Ts[-1][1, 3] + ascale * self.Ts[-1][1, 0]],
            [self.Ts[-1][2, 3], self.Ts[-1][2, 3] + ascale * self.Ts[-1][2, 0]],
            color="r",
        )
        ax.plot(
            [self.Ts[-1][0, 3], self.Ts[-1][0, 3] + ascale * self.Ts[-1][0, 1]],
            [self.Ts[-1][1, 3], self.Ts[-1][1, 3] + ascale * self.Ts[-1][1, 1]],
            [self.Ts[-1][2, 3], self.Ts[-1][2, 3] + ascale * self.Ts[-1][2, 1]],
            color="g",
        )
        ax.plot(
            [self.Ts[-1][0, 3], self.Ts[-1][0, 3] + ascale * self.Ts[-1][0, 2]],
            [self.Ts[-1][1, 3], self.Ts[-1][1, 3] + ascale * self.Ts[-1][1, 2]],
            [self.Ts[-1][2, 3], self.Ts[-1][2, 3] + ascale * self.Ts[-1][2, 2]],
            color="b",
        )
    ...
    def plot_robot(plot, display_mode: str = "gui"):
        if display_mode == "gui":
            ...
            if "microsoft-standard" in platform.uname().release.lower():
                ...
                matplotlib.use("TkAgg")  # Use TkAgg for regular Linux
        else:
        ...
        if display_mode == "gui":
            ...
            plt.savefig("robot_plot.png")
            print("Plot has been saved as 'robot_plot.png'")
